...
extract_info <- function(entry) {
  ...
  year <- str_extract(entry, "\\d{4}")
  set <- str_extract(entry, "(?<=Catalog: Sets:).*")
  
  return(tibble(Pieces = as.integer(pieces), Year = as.integer(year), Set = set))
}
...
scrape_products <- function(base_url, total_pages) {
  ...
  for (page in 1:total_pages) {
    ...
    all_years <- c(all_years, cleaned_data$Year)
    all_sets <- c(all_sets, cleaned_data$Set)
  }
  
  # Crear el dataframe final
  tibble(Name = all_names, Link = all_links, Pieces = all_pieces, Year = all_years, Set = all_sets)
}
...
!!! desempaqueta el vector nombrado en columnas.
set_names(...) asigna nombres de variable.
mutate(...) agrega la moneda en una columna separada.