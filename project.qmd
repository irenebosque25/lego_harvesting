...
The next step consists on applying the function to the rest of the categories.
...
link_product |> 
  ...
  head(1)
```
### Final function for all the products

```{r}
scrape_products <- function(links, output_file, batch_size = 25) {
  # Crear o sobrescribir el archivo CSV con las columnas adecuadas
  write_csv(
    tibble(Link = character(), Year = character(), Times_Sold = character(), Min_Price = character(), 
           Avg_Price = character(), Qty_Avg_Price = character(), Max_Price = character(), Currency = character()),
    output_file
  )

  for (i in seq(1, length(links), by = batch_size)) {
    batch <- links[i:min(i + batch_size - 1, length(links))]  # Seleccionar batch actual

    # Scrapeo y almacenamiento inmediato en CSV
    batch_results <- map_dfr(batch, function(product_url) {
      page <- tryCatch(read_html(product_url), error = function(e) return(NULL))
      if (is.null(page)) return(tibble(Link = product_url, Year = NA, Times_Sold = NA, Min_Price = NA, 
                                       Avg_Price = NA, Qty_Avg_Price = NA, Max_Price = NA, Currency = NA))

      ## 🔹 Extraer año del producto
      product_year <- page %>%
        html_elements("a.links") %>%
        html_text(trim = TRUE) %>%
        head(1)

      ## 🔹 Obtener el identificador del producto para construir el historial de precios
      product_id <- str_extract(product_url, "(?<=S=)[^&]+")  # Extrae el código del set
      if (is.na(product_id)) {
        return(tibble(Link = product_url, Year = product_year, Times_Sold = NA, Min_Price = NA, 
                      Avg_Price = NA, Qty_Avg_Price = NA, Max_Price = NA, Currency = NA))
      }

      # Construir la URL del historial de precios
      price_history_url <- paste0("https://www.bricklink.com/catalogPG.asp?S=", product_id, "&ColorID=0")

      # Leer la página de historial de precios
      price_page <- tryCatch(read_html(price_history_url), error = function(e) return(NULL))
      if (is.null(price_page)) return(tibble(Link = product_url, Year = product_year, Times_Sold = NA, Min_Price = NA, 
                                             Avg_Price = NA, Qty_Avg_Price = NA, Max_Price = NA, Currency = NA))

      ## 🔹 Extraer datos de ventas
      current_items <- price_page %>%
      ...
      # Crear tibble con los datos de ventas
      price_data <- tibble(!!!set_names(numeric_values, titles)) %>%
        mutate(Link = product_url, Year = product_year, Currency = currency) %>%
        select(Link, Year, everything())  # Reordenar columnas

      return(price_data)
    })

    # Guardar en CSV (append para no sobrescribir)
    write_csv(batch_results, output_file, append = TRUE)

    # Mostrar progreso
    message(sprintf("Guardado %d/%d productos...", min(i + batch_size - 1, length(links)), length(links)))
...
scrape_products(first_try$Link, "first_try.csv", batch_size = 25)
```
