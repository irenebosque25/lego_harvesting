...
scrape_years <- function(df) {
  df <- df %>%
    mutate(year = map_chr(Link, function(product_url) {
      ...
      # Extraer el año (primer enlace dentro de la clase 'links')
      product_year <- page %>%
...
get_price_history_links <- function(product_link) {
  # Extraer el código del set de la URL del producto
  product_code <- str_extract(product_link, "S=[A-Za-z0-9-]+")  # Extrae el código del set
  if (is.na(product_code)) {
    return(NA)
  }
  
  # Verificar si el enlace ya contiene "https://"
  price_guide_link <- sprintf("https://www.bricklink.com/catalogPG.asp?%s&ColorID=0", product_code)
  
  # Si el enlace ya contiene "https://", no es necesario agregarlo de nuevo
  if (grepl("^https://", price_guide_link)) {
    return(price_guide_link)
  }
  
  # De lo contrario, concatenamos el dominio
...
Extraemos todos los links de el history price:
...
```{r}
get_price_history_data <- function(price_history_link) {
  
  # Leer la página del historial de precios
  page <- tryCatch(read_html(price_history_link), error = function(e) return(NULL))
  ...
  # Extraer los datos usando los nuevos XPaths
  min_price <- page %>%
    xml_find_first('//*[@id="id-main-legacy-table"]/tbody/tr/td/table[3]/tbody/tr[3]/td[3]/table/tbody/tr/td/table/tbody/tr[3]/td[2]/b') %>%
    html_text(trim = TRUE)
  
  avg_price <- page %>%
    xml_find_first('//*[@id="id-main-legacy-table"]/tbody/tr/td/table[3]/tbody/tr[4]/td[3]/table/tbody/tr/td/table/tbody/tr[3]/td[2]/b') %>%
    html_text(trim = TRUE)
  
  qty_avg_price <- page %>%
    xml_find_first('//*[@id="id-main-legacy-table"]/tbody/tr/td/table[3]/tbody/tr[5]/td[3]/table/tbody/tr/td/table/tbody/tr[3]/td[2]/b') %>%
    html_text(trim = TRUE)
  
  max_price <- page %>%
    xml_find_first('//*[@id="id-main-legacy-table"]/tbody/tr/td/table[3]/tbody/tr[6]/td[3]/table/tbody/tr/td/table/tbody/tr[3]/td[2]/b') %>%
    html_text(trim = TRUE)
  
  # Extraer la moneda (aquí puedes ajustarlo si es necesario)
  currency <- "USD"  # Aquí puedes poner un código de moneda predeterminado o extraerlo si lo ves en la página
  
  # Devolver los datos como un tibble
  tibble(
    Min_Price = min_price,
    Avg_Price = avg_price,
...
```{r}
first_try <- all_products[1:25,]

first_try <- first_try %>%
  rowwise() %>%
  mutate(
    price_data = list(get_price_history_data(price_history_link)),
    Min_Price = price_data[[1]]$Min_Price,
    Avg_Price = price_data[[1]]$Avg_Price,
    Qty_Avg_Price = price_data[[1]]$Qty_Avg_Price,
    Max_Price = price_data[[1]]$Max_Price,
    Currency = price_data[[1]]$Currency
  ) %>%
  select(-price_data) 
```
...
first_try <- first_try %>% 
  mutate(price_history = map(Link, get_price_history_links))
```
...
```{r}
# Función para extraer y limpiar los precios de un enlace de historial
extract_price_history <- function(url) {
  page <- read_html(url)
  
  # Extraer los datos de la tabla
  current_items <- page |> 
    xml_find_all("(//table[@class='fv']//td[@valign='TOP'])[3]") |> 
    html_text()
  ...
  # Crear un tibble con los datos extraídos
  result <- tibble(!!!set_names(numeric_values, titles)) |> 
    mutate(Currency = currency)
  
  return(result)
}

# Aplicar la función a todas las filas de la columna price_history_link y combinar los resultados
apply_scraping <- function(dataset) {
  dataset_with_prices <- dataset |> 
    rowwise() |> 
    mutate(price_data = list(extract_price_history(price_history_link))) |> 
    unnest(price_data)
  
  return(dataset_with_prices)
}

# Usar la función con tu dataset
new_dataset <- apply_scraping(all_products)
```

```{r}
subset_products <- head(all_products, 30)

# Función para extraer el enlace de la guía de precios
get_price_guide_link <- function(url) {
  tryCatch({
    page <- read_html(url)
    link <- page %>%
      html_nodes(xpath = "//div[@id='_idPriceGuideLink']//a") %>%
      html_attr("href")
    
    # Retornar el enlace o NA si no se encuentra
    if (length(link) > 0) {
      return(link)
    } else {
      return(NA)
...
# Aplicar la función a cada fila del dataset (solo los primeros 30 productos)
subset_products <- subset_products %>%
  mutate(PriceGuideLink = sapply(Link, get_price_guide_link))

# Guardar el nuevo dataset en un archivo CSV
write.csv(subset_products, "subset_products.csv", row.names = FALSE)
