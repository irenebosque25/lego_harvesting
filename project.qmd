...
```{r}
write.csv(all_products, "years_products.csv", row.names = FALSE)
```
...
all_products3 <- all_products |> 
  left_join(all_products2, by="Name")

all_products_clean <- all_products3 |> 
  drop_na(price_link) 
...
```{r}
link <- "https://www.bricklink.com/v2/catalog/catalogitem.page?S=3342-1#T=S&O={%22iconly%22:0}" |> 
  read_html()

link |> 
  xml_find_all("//tr[@class='pciItemHeader']//td[@width='90px']") |> 
  html_text()
```
...
first_try <- all_products_clean |> 
  ...
  filter(year == "2000"|year == "2014"|year == "2017") |> 
  head(20)
...
```{r}
first_try <- all_products_clean |> 
  filter(category_name == "Super Mario Bros") |> 
    filter(Years == "1990") |> 
  head(30)
...
```{r}
write.csv(products1, "SuperMario_products.csv", row.names = FALSE)
```
